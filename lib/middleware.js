...
var instrumenterESModules = istanbul.createInstrumenter({
  coverageVariable: '__coverage__',
  preserveComments: true,
  compact: false,
  autoWrap: true,
...
function instrumentHtml(html, req, root){
  ...
  function gotScript(code, loc) {
    let instrumented;
    try {
      instrumented = instrumenter.instrumentSync(code, htmlFilePath);
    }
    catch (e) {
      if (e.message.includes(`'import' and 'export' may appear only with`)) {
        instrumented = instrumenterESModules.instrumentSync(code, htmlFilePath);
      }
      else {
        console.log('path = ' + htmlFilePath, 'code = \n', code);
        throw e;
      }
    }
...
function instrumentAsset(code, req, root){
  ...
  if ( !cache[asset] ){
    let instrumented;
    try {
      instrumented = instrumenter.instrumentSync(code, assetPath);
    }
    catch (e) {
      if (e.message.includes(`'import' and 'export' may appear only with`)) {
        instrumented = instrumenterESModules.instrumentSync(code, assetPath);
      }
      else {
        console.log('path = ' + assetPath, 'code = \n', code);
        throw e;
      }
    }
    cache[asset] = patchCoverageVariable(instrumented);
  }
 ...
 */
function coverageMiddleware(root, options, emitter, chain = false, last = false) {
